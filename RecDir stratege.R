...
TZ<-function(imf){
  ...
  for (i in 1:24) {#i=1代表第一步
    ...
    fit=function(x) {#适应度函数
      ...
      mape=mean((predict(model,validX)-validy[,i])^2)
      return (mape)
    }
    ps=psoptim(par = rep(NA,2),function(x) fit(x),
               lower = c(0.01,0.001),upper = c(1000,100),control = list(maxit=100))
    ...
    trainx=cbind(trainx,trainy[,i])
    validx=cbind(validx,validy[,i])
